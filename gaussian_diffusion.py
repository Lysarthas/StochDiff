...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, cond, t, clip_denoised: bool):
        if self.pred_type == "data":
            x_recon = self.denoise_fn(x, t, conds=cond)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, cond, sampling_timesteps, eta=0., clip_denoised = True):
        ...
        for time, time_next in time_pairs:
            ...
            if self.pred_type == "data":
                x_start = self.denoise_fn(img, time_cond, cond)
                pred_noise = self.predict_noise_from_start(img, time_cond, x_start)
            elif self.pred_type == "noise":
                pred_noise = self.denoise_fn(img, time_cond, cond)
    ...
    @torch.no_grad()
    def sample(self, sample_shape=torch.Size(), cond=None, sampling_timesteps=None):
        if cond is not None:
        ...
        if sampling_timesteps is not None:
            x_hat = self.ddim_sample(shape, cond, sampling_timesteps)
        else:
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        x_recon = self.denoise_fn(x_noisy, t, conds=cond)

        if self.pred_type == "data":
            if kwargs['fore']:
                loss = F.nll_loss(x_recon, x_start)
            else:
    ...
    def log_prob(self, x, cond, *args, **kwargs):
        ...
        Bx, _ = x.shape
        time = torch.randint(0, self.num_timesteps, (Bx,), device=x.device).long()
        loss = self.p_losses(
